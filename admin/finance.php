...
$paymentRequestEvents=$pdo->query("SELECT id,title,event_date FROM events ORDER BY event_date DESC,id DESC")->fetchAll()?:[];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($action==='create_event_payout'){
        ...
        $email=strtolower(trim((string)($_POST['recipient_email']??''))); $description=trim((string)($_POST['description']??'')); $amount=price_to_number($_POST['amount']??0); $eventId=max(0,(int)($_POST['event_id']??0)); $entryId=max(0,(int)($_POST['booking_item_id']??0));
        ...
        elseif($eventId>0&&!fetchEventById($pdo,$eventId))$alerts[]=['type'=>'danger','message'=>'Selected event was not found.'];
        else { if($entryId>0){$entryCheck=$pdo->prepare('SELECT event_id FROM booking_items WHERE id=:id LIMIT 1');$entryCheck->execute([':id'=>$entryId]);$entryEventId=(int)($entryCheck->fetchColumn()?:0);if($entryEventId<=0||($eventId>0&&$eventId!==$entryEventId)){$alerts[]=['type'=>'danger','message'=>'Selected entry does not belong to the selected event.'];}} if(!$alerts){if($entryId>0)$eventId=$entryEventId; $token=bin2hex(random_bytes(24)); $insert=$pdo->prepare("INSERT INTO misc_payment_requests (request_token,recipient_email,description,amount,currency,status,event_id,booking_item_id,created_by_user_id) VALUES (:token,:email,:description,:amount,:currency,'sent',:event,:entry,:user)"); $insert->execute([':token'=>$token,':email'=>$email,':description'=>substr($description,0,255),':amount'=>number_format($amount,2,'.',''),':currency'=>$stripeConfig['currency']??'gbp',':event'=>$eventId?:null,':entry'=>$entryId?:null,':user'=>(int)$currentUser['id']]); $requestId=(int)$pdo->lastInsertId();
            ...
            else { $pdo->prepare('UPDATE misc_payment_requests SET stripe_session_id=:session,stripe_checkout_url=:url,email_sent_at=NOW() WHERE id=:id')->execute([':session'=>$sessionId,':url'=>$checkoutUrl,':id'=>$requestId]); $settings=getEmailSettings($pdo); $siteSettings=getSiteSettings($pdo); $inner='<p>Please use the secure link below to pay <strong>'.h(format_price($amount)).'</strong>.</p><p>'.nl2br(h($description)).'</p><div style="margin:16px 0;">'.email_cta_button_html($checkoutUrl,'Pay securely').'</div><p>If the button does not work, copy this link into your browser:<br><a href="'.h($checkoutUrl).'">'.h($checkoutUrl).'</a></p>'; $sent=send_logged_email($pdo,$email,'Payment request: '.substr($description,0,120),wrap_user_email_html($siteSettings,$settings,$inner),"Payment request\n\n".$description."\nAmount: ".format_price($amount)."\nPay securely: ".$checkoutUrl,['type'=>'misc_payment_request','misc_payment_request_id'=>$requestId,'stripe_session_id'=>$sessionId]); if(!$sent)$alerts[]=['type'=>'warning','message'=>'The Stripe link was created, but the email could not be sent. Check Email logs.']; else $successMessage='Payment request emailed to '.$email.'.'; }
        }}
        ...
        else{$stmt=$pdo->prepare("SELECT * FROM misc_payment_requests WHERE id=:id AND status='sent' LIMIT 1");$stmt->execute([':id'=>$requestId]);$request=$stmt->fetch()?:null;if(!$request||empty($request['stripe_checkout_url']))$alerts[]=['type'=>'danger','message'=>'This payment request cannot be reminded.'];else{$settings=getEmailSettings($pdo);$siteSettings=getSiteSettings($pdo);$url=(string)$request['stripe_checkout_url'];$inner='<p>This is a reminder that payment of <strong>'.h(format_price((float)$request['amount'])).'</strong> is now due.</p><p>'.nl2br(h((string)$request['description'])).'</p><div style="margin:16px 0;">'.email_cta_button_html($url,'Pay now').'</div><p>If the button does not work, copy this link into your browser:<br><a href="'.h($url).'">'.h($url).'</a></p>';if(send_logged_email($pdo,(string)$request['recipient_email'],'Reminder: payment now due — '.substr((string)$request['description'],0,100),wrap_user_email_html($siteSettings,$settings,$inner),"Payment reminder\n\nPayment is now due.\n".$request['description']."\nAmount: ".format_price((float)$request['amount'])."\nPay now: ".$url,['type'=>'misc_payment_request_reminder','misc_payment_request_id'=>$requestId])){$pdo->prepare('UPDATE misc_payment_requests SET email_sent_at=NOW() WHERE id=:id')->execute([':id'=>$requestId]);$successMessage='Payment reminder emailed.';}else $alerts[]=['type'=>'warning','message'=>'Reminder could not be sent. Check Email history.'];}}
        if($alerts)$_SESSION['flash_alerts']=$alerts;if($successMessage)$_SESSION['flash_success']=$successMessage;header('Location: finance.php?tab=requests');exit;
...
<div class="mt-2 d-flex flex-wrap gap-2"><?php foreach($miscPaymentRequests as $request): if((string)$request['status']!=='sent')continue; ?><form method="post"><input type="hidden" name="action" value="resend_misc_payment_request"><input type="hidden" name="csrf" value="<?php echo h($miscPaymentCsrf); ?>"><input type="hidden" name="request_id" value="<?php echo (int)$request['id']; ?>"><button class="btn btn-sm btn-outline-warning">Send reminder: <?php echo h((string)$request['recipient_email']); ?></button></form><?php endforeach; ?></div>
<div class="modal fade" id="newMiscPaymentModal" tabindex="-1" aria-labelledby="newMiscPaymentModalLabel" aria-hidden="true"><div class="modal-dialog"><form method="post" class="modal-content"><input type="hidden" name="action" value="create_misc_payment"><input type="hidden" name="csrf" value="<?php echo h($miscPaymentCsrf); ?>"><input type="hidden" name="booking_item_id" value="<?php echo $paymentRequestPrefillEntryId?:''; ?>"><div class="modal-header"><h5 class="modal-title" id="newMiscPaymentModalLabel">New payment request</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div><div class="modal-body"><p class="small text-muted">The recipient receives a secure Stripe-hosted payment link by email.</p><div class="mb-3"><label class="form-label">Event / ride (optional)</label><select class="form-select" name="event_id"><option value="">Not linked to an event</option><?php foreach($paymentRequestEvents as $requestEvent): ?><option value="<?php echo (int)$requestEvent['id']; ?>" <?php echo $paymentRequestPrefillEventId===(int)$requestEvent['id']?'selected':''; ?>><?php echo h(format_display_date($requestEvent['event_date']??null,'').' · '.(string)$requestEvent['title']); ?></option><?php endforeach; ?></select></div><div class="mb-3"><div class="d-flex justify-content-between align-items-center gap-2"><label class="form-label mb-1" for="misc-recipient-email">Recipient email</label><button class="btn btn-sm btn-outline-success" type="button" id="open-recipient-lookup">Find existing user / person</button></div><input class="form-control" id="misc-recipient-email" type="email" name="recipient_email" value="<?php echo h($paymentRequestPrefillEmail); ?>" required><div class="form-text">Or type any email address directly.</div></div><div class="mb-3"><label class="form-label">Description</label><input class="form-control" name="description" maxlength="255" required></div><div><label class="form-label">Amount</label><div class="input-group"><span class="input-group-text">£</span><input class="form-control" type="number" name="amount" min="0.01" step="0.01" required></div></div></div><div class="modal-footer"><button class="btn btn-outline-secondary" type="button" data-bs-dismiss="modal">Cancel</button><button class="btn btn-success">Email payment request</button></div></form></div></div>
