...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($csrf, (string)($_POST['csrf'] ?? ''))) {
        ...
        if ($action === 'create') {
            ...
            $status = in_array($requestedStatus, ['open','completed','closed'], true) ? $requestedStatus : 'open';
            ...
            $_SESSION['flash_success']=$status==='closed'?'Task closed.':($status==='completed'?'Task marked completed and ready for review.':'Task reopened.'); header('Location: dev_task.php?id='.$taskId); exit;
