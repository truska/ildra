...
<div id="conversation">
<?php foreach($messages as $message): ?><article class="card-soft p-3 mb-3 dev-task-conversation-item"><div class="d-flex justify-content-between gap-3 mb-2"><strong><?php echo h($message['author_name']); ?></strong><time class="small text-muted text-nowrap"><?php echo h(date('j M Y, H:i',strtotime($message['created_at']))); ?></time></div><div style="white-space:pre-wrap"><?php echo h($message['message']); ?></div><?php if(!empty($message['image_filename'])): ?><a href="<?php echo h(image_upload_public_path('dev-tasks','original',$message['image_filename'])); ?>" target="_blank" rel="noopener" class="d-inline-block mt-3"><img class="img-fluid rounded border" style="max-height:420px" src="<?php echo h(image_upload_public_path('dev-tasks','lg',$message['image_filename'])); ?>" alt="Screenshot attached by <?php echo h($message['author_name']); ?>"></a><?php endif; ?></article><?php endforeach; ?>
</div>
