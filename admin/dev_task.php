...
<?php elseif ($task): ?>
<style>
    .dev-task-notes { float:right; width:calc(33.333% - .75rem); margin:0 0 1rem 1.5rem; }
    .dev-task-conversation-item { display:flow-root; }
    @media (max-width: 767.98px) { .dev-task-notes { float:none; width:auto; margin:0 0 1rem; } }
</style>
...
<aside class="dev-task-notes card-soft p-3" id="task-notes"><h6 class="mb-2">Task notes</h6><p class="small text-muted mb-2">A shared running list for this task.</p><form method="post"><input type="hidden" name="csrf" value="<?php echo h($csrf); ?>"><input type="hidden" name="action" value="save_notes"><input type="hidden" name="task_id" value="<?php echo $taskId; ?>"><textarea class="form-control" name="task_notes" rows="12" placeholder="Add notes, checklist items or decisions…"><?php echo h((string)($task['task_notes'] ?? '')); ?></textarea><button class="btn btn-sm btn-success mt-2">Save notes</button></form></aside>
<div id="conversation">
<?php foreach($messages as $message): ?><article class="card-soft p-3 mb-3 dev-task-conversation-item"><div class="d-flex justify-content-between gap-3 mb-2"><strong><?php echo h($message['author_name']); ?></strong><time class="small text-muted text-nowrap"><?php echo h(date('j M Y, H:i',(int)$message['created_at_ts'])); ?></time></div><div style="white-space:pre-wrap"><?php echo h($message['message']); ?></div><?php if(!empty($message['image_filename'])): ?><a href="<?php echo h(image_upload_public_path('dev-tasks','original',$message['image_filename'])); ?>" target="_blank" rel="noopener" class="d-inline-block mt-3"><img class="img-fluid rounded border" style="max-height:420px" src="<?php echo h(image_upload_public_path('dev-tasks','lg',$message['image_filename'])); ?>" alt="Screenshot attached by <?php echo h($message['author_name']); ?>"></a><?php endif; ?></article><?php endforeach; ?>
</div>
