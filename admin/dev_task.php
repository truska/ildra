    ...
    if (!hash_equals($csrf, (string)($_POST['csrf'] ?? ''))) {
        ...
        if ($action === 'create') {
            ...
            if (devTaskSaveNotes($pdo, $taskId, (string)($_POST['task_notes'] ?? ''), $currentUser)) {
                $_SESSION['flash_success']='Task notes updated.'; header('Location: dev_task.php?id='.$taskId.'#task-notes'); exit;
            }
            ...
            if (!$alerts) {
                ...
                $_SESSION['flash_success']='Task updated.'; header('Location: dev_task.php?id='.$taskId); exit;
            }
...
$task = null; $messages=[];
if ($taskId > 0) {
    $stmt=$pdo->prepare('SELECT *,UNIX_TIMESTAMP(created_at) AS created_at_ts FROM dev_tasks WHERE id=:id'); $stmt->execute([':id'=>$taskId]); $task=$stmt->fetch() ?: null;
    if ($task) { $stmt=$pdo->prepare('SELECT *,UNIX_TIMESTAMP(created_at) AS created_at_ts FROM dev_task_messages WHERE task_id=:id ORDER BY created_at,id'); $stmt->execute([':id'=>$taskId]); $messages=$stmt->fetchAll() ?: []; }
...
<?php elseif ($task): ?>
<style>
    .dev-task-notes { min-height:320px; }
...
<div class="col-md-8">
<?php if ($messages): $message=$messages[0]; ?><article class="card-soft p-3"><div class="d-flex justify-content-between gap-3 mb-2"><strong><?php echo h($message['author_name']); ?></strong><time class="small text-muted text-nowrap"><?php echo h(date('j M Y, H:i',(int)$message['created_at_ts'])); ?></time></div><div style="white-space:pre-wrap"><?php echo h($message['message']); ?></div><?php if(!empty($message['image_filename'])): ?><a href="<?php echo h(image_upload_public_path('dev-tasks','original',$message['image_filename'])); ?>" target="_blank" rel="noopener" class="d-inline-block mt-3"><img class="img-fluid rounded border" style="max-height:420px" src="<?php echo h(image_upload_public_path('dev-tasks','lg',$message['image_filename'])); ?>" alt="Screenshot attached by <?php echo h($message['author_name']); ?>"></a><?php endif; ?></article><?php endif; ?>
</div>
<div class="col-md-4"><aside class="dev-task-notes card-soft p-3" id="task-notes"><h6 class="mb-2">Task notes</h6><p class="small text-muted mb-2">A shared running list for this task.</p><form method="post"><input type="hidden" name="csrf" value="<?php echo h($csrf); ?>"><input type="hidden" name="action" value="save_notes"><input type="hidden" name="task_id" value="<?php echo $taskId; ?>"><textarea class="form-control" name="task_notes" rows="12" placeholder="Add notes, checklist items or decisions…"><?php echo h((string)($task['task_notes'] ?? '')); ?></textarea><button class="btn btn-sm btn-success mt-2">Save notes</button></form></aside></div>
</div>
<?php foreach(array_slice($messages,1) as $message): ?><article class="card-soft p-3 mb-3"><div class="d-flex justify-content-between gap-3 mb-2"><strong><?php echo h($message['author_name']); ?></strong><time class="small text-muted text-nowrap"><?php echo h(date('j M Y, H:i',(int)$message['created_at_ts'])); ?></time></div><div style="white-space:pre-wrap"><?php echo h($message['message']); ?></div><?php if(!empty($message['image_filename'])): ?><a href="<?php echo h(image_upload_public_path('dev-tasks','original',$message['image_filename'])); ?>" target="_blank" rel="noopener" class="d-inline-block mt-3"><img class="img-fluid rounded border" style="max-height:420px" src="<?php echo h(image_upload_public_path('dev-tasks','lg',$message['image_filename'])); ?>" alt="Screenshot attached by <?php echo h($message['author_name']); ?>"></a><?php endif; ?></article><?php endforeach; ?>
</div>
