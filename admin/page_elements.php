...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reorder') {
        $posted=(array)($_POST['order']??[]);$allowed=array_column(fetchPageContentElements($pdo,$pageId),null,'id');
        if(count($posted)!==count($allowed)){$_SESSION['flash_alerts']=[['type'=>'danger','message'=>'The section list changed before the order was saved. Please try again.']];}
        else{try{$pdo->beginTransaction();$stmt=$pdo->prepare('UPDATE page_content_elements SET display_order=:ord WHERE id=:id AND page_id=:page');foreach($posted as$elementId=>$order){$elementId=(int)$elementId;if(!isset($allowed[$elementId]))throw new RuntimeException('Invalid section');$stmt->execute([':ord'=>(int)$order,':id'=>$elementId,':page'=>$pageId]);}$pdo->commit();$_SESSION['flash_success']='Section order saved.';}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();$_SESSION['flash_alerts']=[['type'=>'danger','message'=>'Could not save the section order.']];}}
        header('Location: page_elements.php?page_id='.$pageId);exit;
    }
    if ($action === 'delete') {
...
<?php else: ?>
<div class="card-soft p-3"><form method="post"><input type="hidden" name="page_id" value="<?php echo $pageId; ?>"><input type="hidden" name="action" value="reorder"><div class="table-responsive"><table class="table align-middle" data-admin-inline-reorder><thead><tr><th>Name / heading</th><th>Type</th><th>Status</th><th>Order</th><th class="text-end">Actions</th></tr></thead><tbody><?php foreach ($elements as $element): ?><?php $typeLabels=['membership_options'=>'Membership options','horse_logbook_information'=>'Horse logbook information','ride_prices'=>'Ride prices','current_events_calendar'=>'Current events calendar','past_events_calendar'=>'Past events calendar','faqs'=>'FAQs','news_list'=>'News list','awards_winners'=>'Awards and past winners']; ?><tr data-order-id="<?php echo (int)$element['id']; ?>"><td><strong><?php echo h($element['name']); ?></strong><div class="small text-muted"><?php echo h($element['heading'] ?: ''); ?></div></td><td><?php echo h($typeLabels[$element['content_type'] ?? 'rich_text'] ?? 'Rich text'); ?></td><td><?php echo !empty($element['archived']) ? 'Archived' : (!empty($element['show_on_web']) ? 'Visible' : 'Hidden'); ?></td><td><input class="form-control form-control-sm" style="width:90px" data-display-order type="number" name="order[<?php echo (int)$element['id']; ?>]" value="<?php echo (int)$element['display_order']; ?>"></td><td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="page_elements.php?page_id=<?php echo $pageId; ?>&edit=<?php echo (int)$element['id']; ?>">Edit</a> <a class="btn btn-sm btn-outline-primary" href="element_images.php?element_id=<?php echo (int)$element['id']; ?>">Images</a> <button class="btn btn-sm btn-outline-danger" type="submit" name="id" value="<?php echo (int)$element['id']; ?>" onclick="this.form.elements.action.value='delete';return confirm('Delete this section?');">Delete</button></td></tr><?php endforeach; ?><?php if (!$elements): ?><tr><td colspan="5" class="text-muted">No additional content sections yet.</td></tr><?php endif; ?></tbody></table></div><?php if($elements):?><button class="btn btn-success">Save section order</button><?php endif;?></form></div>
