...
$tableColumns = [
    ...
    'next_action' => ['label'=>'Next action by','sortable'=>true,'filter'=>'select','options'=>$nextActionOptions,'form'=>$filterForm,'value'=>static fn(array $row):string=>(string)($row['next_action_by'] ?? 0),'sort_value'=>static fn(array $row):string=>trim(($row['assignee_first_name']??'').' '.($row['assignee_last_name']??'')) ?: ($row['assignee_email']??'Unassigned')],
...
$table = admin_table_prepare($tasks, $tableColumns, 'priority');
$tasks = $table['rows'];
...
<div class="card-soft p-3"><?php echo admin_table_record_count($table,'task','tasks'); ?><div class="table-responsive"><table class="table table-sm admin-data-table align-middle mb-0">
...
</tbody></table></div><?php echo admin_table_pagination($table); ?><div class="mt-3 text-end"><a class="btn btn-sm btn-outline-secondary" href="dev_tasks.php?status=<?php echo h($filter); ?>">Clear filters</a></div></div>
