...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postMaxBytes > 0 && (int)($_SERVER['CONTENT_LENGTH'] ?? 0) > $postMaxBytes) {
        ...
        if ($action === 'save') {
            ...
            $uploadError=null; $stored=rideNotesStorePdf($_FILES['ride_notes_pdf'] ?? [], $event, $uploadError);
            if (!$stored) $alerts[]=['type'=>'danger','message'=>$uploadError ?: 'Unable to upload the Ride Notes PDF.'];
            elseif (saveRideNotesPdf($pdo, $eventId, $stored)) { $_SESSION['flash_success']='Ride Notes PDF uploaded.'; header('Location: ride_notes.php?event_id='.$eventId); exit; }
            else $alerts[]=['type'=>'danger','message'=>'The PDF was uploaded but could not be recorded against this Ride Notes page.'];
            ...
            elseif (!$recipients) $alerts[]=['type'=>'danger','message'=>'Enter one or more valid test email addresses.'];
                ...
                $emailSettings=getEmailSettings($pdo); $payload=rideNotesEmailPayload($event, $notes, $siteSettings, $emailSettings, $siteBase); $attachment=rideNotesPdfAttachment($notes); $attachments=$attachment?[$attachment]:[];
                $sent=0; foreach($recipients as $recipient) if(send_logged_email($pdo,$recipient,$payload['subject'],$payload['html'],$payload['text'],['kind'=>'ride_notes_test','event_id'=>$eventId,'admin_user_id'=>(int)$currentUser['id']],$attachments)) $sent++;
