...
function ensureEmailCampaignTables(?PDO $pdo): void
{
    ...
    $pdo->exec("INSERT IGNORE INTO email_campaign_templates (template_key,name,category,renderer_key,audience_preset,intro_html,outro_html,subject_template,html_template,text_template,is_system,is_active) VALUES ('weekly_ride_notice','Weekly Ride Notice','ride_notice','weekly_ride_notice','all_users','<p>Hello {{first_name}},</p><p>Here are the next rides and events. Dates and details can change, so please check the website for the latest information.</p>','','Ride Notice - {{current_date}}','{{weekly_intro}}','{{weekly_intro_text}}',1,1)");
    $pdo->exec("UPDATE email_campaign_templates SET renderer_key='weekly_ride_notice', intro_html=CASE WHEN intro_html IS NULL OR intro_html='' OR intro_html LIKE '%Here are the next rides and events. Dates and details can change, so please check the website for the latest information.%Forthcoming Rides and Events%' OR intro_html LIKE '%Forthcoming Rides and Events%Here are the next rides and events. Dates and details can change, so please check the website for the latest information.%' THEN '<p>Hello {{first_name}},</p><p>Here are the next rides and events. Dates and details can change, so please check the website for the latest information.</p>' ELSE intro_html END, outro_html=COALESCE(outro_html,'') WHERE template_key='weekly_ride_notice'");
...
function weeklyRideNoticeContent(PDO $pdo, string $baseUrl): array
{
    ...
    if(!$rows)return ['html'=>'{{weekly_intro}}<h1 style="margin:24px 0 16px;color:#0c2a12;font-size:26px;line-height:1.2;">Forthcoming Rides and Events</h1><p>There are no upcoming events to include this week.</p>{{weekly_outro}}','text'=>"{{weekly_intro_text}}\n\nForthcoming Rides and Events\n\nThere are no upcoming events to include this week.\n\n{{weekly_outro_text}}"];
    ...
    $calendarUrl=$baseUrl.'/events';
    $html='{{weekly_intro}}<h1 style="margin:24px 0 16px;color:#0c2a12;font-size:26px;line-height:1.2;">Forthcoming Rides and Events</h1><h2 style="color:#0c2a12;">Featured event</h2>'.$card($rows[0],true).(count($rows)>1?'<h2 style="color:#0c2a12;">More upcoming events</h2>':'');foreach(array_slice($rows,1) as $event)$html.=$card($event,false);$html.='<p style="margin-top:20px;">'.email_cta_button_html($calendarUrl,'View full calendar').'</p><div style="margin-top:12px;color:#476146;font-size:13px;line-height:1.5;">If the button does not work, copy and paste this link into your browser:<br><a href="'.h($calendarUrl).'" style="color:#146118;word-break:break-all;">'.h($calendarUrl).'</a></div>{{weekly_outro}}';
    $text="{{weekly_intro_text}}\n\nForthcoming Rides and Events\n\n";foreach($rows as $event)$text.=(string)$event['title']."\n".format_display_date($event['event_date']??null,'')." · ".(string)($event['venue']??'')."\n".$baseUrl.'/event.php?id='.(int)$event['id']."\n\n";$text.="Full calendar: {$calendarUrl}\n\nIf the button does not work, copy and paste this link into your browser:\n{$calendarUrl}\n\n{{weekly_outro_text}}";return ['html'=>$html,'text'=>$text];
}
...
function ensureWeeklyRideNoticeCampaign(PDO $pdo): void
{
    ...
    $insert=$pdo->prepare("INSERT INTO email_campaigns (name,campaign_type,category,template_id,audience_preset,membership_year,address_strategy,renderer_key,intro_html,outro_html,subject_template,html_template,text_template,status,scheduled_at,batch_size,live_send_approved) VALUES ('Weekly Ride Notice','weekly_ride_notice','ride_notice',:template,'all_users',:year,'person_first','weekly_ride_notice',:intro,:outro,:subject,:html,:text,'scheduled',:scheduled,:batch,1)");$insert->execute([':template'=>(int)$template['id'],':year'=>(int)$next->format('Y'),':intro'=>(string)($template['intro_html']??''),':outro'=>(string)($template['outro_html']??''),':subject'=>(string)$template['subject_template'],':html'=>$body['html'],':text'=>$body['text'],':scheduled'=>$scheduled,':batch'=>max(1,(int)($settings['campaign_default_batch_size']??25))]);
}
...
function emailCampaignRenderRecipient(array $campaign,array $recipient,array $settings): array
{
    ...
    $intro=emailCampaignMerge((string)($campaign['intro_html']??''),$merge); $outro=emailCampaignMerge((string)($campaign['outro_html']??''),$merge);
    $toText=static function(string $html): string { return trim(html_entity_decode(strip_tags(preg_replace('/<(?:br\\s*\\/?|\\/p|\\/div|\\/h[1-6])[^>]*>/i', "\n", $html) ?? $html), ENT_QUOTES | ENT_HTML5, 'UTF-8')); };
    $merge['weekly_intro']=$intro; $merge['weekly_outro']=$outro;
    $merge['weekly_intro_text']=$toText($intro); $merge['weekly_outro_text']=$toText($outro);
