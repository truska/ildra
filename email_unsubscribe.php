...
$token=trim((string)($_GET['t']??$_POST['t']??''));$preview=($_GET['preview']??'')==='1';$recipient=null;$done=false;$category='general';
if($pdo&&preg_match('/^[a-f0-9]{40}$/',$token)){
    ...
    if(!$recipient){$stmt=$pdo->prepare('SELECT t.*,c.name campaign_name FROM email_campaign_limited_tests t JOIN email_campaigns c ON c.id=t.campaign_id WHERE t.unsubscribe_token=:token LIMIT 1');$stmt->execute([':token'=>$token]);$recipient=$stmt->fetch()?:null;}
}
...
$preferenceLabel=match($category){'ride_notice'=>'the weekly Ride Notice','renewal_reminder'=>'renewal reminders',default=>'general news and announcements'};
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Email preferences</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light"><main class="container py-5" style="max-width:680px"><div class="card shadow-sm"><div class="card-body p-4 p-md-5"><h1 class="h3">Email preferences</h1><?php if($preview): ?><div class="alert alert-info mt-3 mb-0">This is a test email preference link. No subscription preferences can be changed from a test email.</div><?php elseif(!$recipient): ?><div class="alert alert-warning mt-3">This email preference link is invalid or has expired.</div><?php elseif($category==='operational'): ?><div class="alert alert-info mt-3">This was an essential message about an event or service connected to you. Operational messages are not part of marketing subscriptions.</div><?php elseif($done): ?><div class="alert alert-success mt-3">You have been unsubscribed from <?php echo h($preferenceLabel); ?>.</div><?php else: ?><p>This will unsubscribe <strong><?php echo h((string)$recipient['email']); ?></strong> from <?php echo h($preferenceLabel); ?>.</p><form method="post"><input type="hidden" name="t" value="<?php echo h($token); ?>"><button class="btn btn-danger">Unsubscribe</button></form><?php endif; ?></div></div></main></body></html>
